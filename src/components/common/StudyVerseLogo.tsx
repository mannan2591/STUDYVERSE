...
export const StudyVerseLogo: React.FC<LogoProps> = ({
  ...
  const sizeMap = {
    sm: { icon: 32, text: 'text-lg', tag: 'text-[9px]', height: 32 },
    md: { icon: 44, text: 'text-xl', tag: 'text-[11px]', height: 44 },
    lg: { icon: 56, text: 'text-2xl', tag: 'text-xs', height: 56 },
    xl: { icon: 72, text: 'text-3xl', tag: 'text-sm', height: 72 },
    hero: { icon: 96, text: 'text-4xl', tag: 'text-base', height: 96 },
  };

  const dim = sizeMap[size];

  // SVG Icon Component mirroring the authentic brand guide
  const BrandIcon = ({ width = dim.icon, height = dim.icon }: { width?: number; height?: number }) => (
    ...
    >
      ...
      {/* Open Book Base */}
      <path
        d="M 28 132 Q 54 122 80 134 Q 106 122 132 132 Q 120 144 80 144 Q 40 144 28 132 Z"
        fill="#075E48"
      />
      <path
        d="M 32 130 Q 56 120 80 131 Q 104 120 128 130 Q 116 140 80 140 Q 44 140 32 130 Z"
        fill="#F7F4EA"
      />
      {/* Book Center spine highlight */}
      <path d="M 78 131 L 82 131 L 82 142 L 78 142 Z" fill="#E6A83A" />

      {/* Stylized 'S' Monogram in Cream */}
      <path
      ...
      {/* Graduation Cap on top of S */}
      <g transform="translate(42, 14)" filter="url(#subtleShadow)">
        {/* Cap Diamond */}
      ...
      {/* Pen forming the 'V' Stroke on right */}
      <g transform="translate(80, 48)" filter="url(#subtleShadow)">
        {/* Pen Barrel (Green) */}
        <path
          d="M 28 8 L 44 26 L 24 58 L 8 40 Z"
  ...
  if (variant === 'horizontal') {
    return (
      <div className={`inline-flex items-center gap-3 ${className}`}>
        <BrandIcon width={Math.max(34, dim.icon * 0.8)} height={Math.max(34, dim.icon * 0.8)} />
        <div className="flex flex-col">
          <div className={`font-bold tracking-tight ${dim.text} leading-none flex items-center`}>
            <span className="text-[#171A19] dark:text-[#F7F4EA]">Study</span>
            <span className="text-[#0F8B6D]">Verse</span>
          </div>
